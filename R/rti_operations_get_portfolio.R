...
#' Перевеод в число объекта MoneyValue
#' 
#' @export
...
as.data.frame.tinkoff.invest.schemas.PortfolioResponse <- function(x){
    ...
    attr(df, "total_amount_currencies") <- as.numeric(x$total_amount_currencies)
    attr(df, "total_amount_bonds") <- as.numeric(x$total_amount_bonds)
    attr(df, "total_amount_etf") <- as.numeric(x$total_amount_etf)
    attr(df, "total_amount_shares") <- as.numeric(x$total_amount_shares)
    attr(df, "total_amount_futures") <- as.numeric(x$total_amount_futures)
